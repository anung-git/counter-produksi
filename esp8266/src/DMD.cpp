...
DMD::DMD(byte panelsWide, byte panelsHigh, byte sck, byte strb, byte dataR, byte enable)
{
    this->pin_oe = enable;
    this->pin_sck = sck;
    this->pin_dataR = dataR;
    this->pin_strb = strb;
    ...
    digitalWrite(this->pin_oe, LOW);     // enable;
    digitalWrite(this->pin_sck, LOW);    // sck;
    digitalWrite(this->pin_dataR, HIGH); // dataR;
    digitalWrite(this->pin_strb, LOW);   // strb;

    pinMode(this->pin_oe, OUTPUT);    // enable;
    pinMode(this->pin_sck, OUTPUT);   // sck;
    pinMode(this->pin_dataR, OUTPUT); // dataR;
    pinMode(this->pin_strb, OUTPUT);  // strb;

    // analogWrite(pin_oe, 200);
...
void shiftOutBeep(uint8_t dataPin, uint8_t clockPin, uint8_t val)
{
...
void DMD::shiftOut(uint8_t dataPin, uint8_t clockPin, uint8_t val)
{
...
void DMD::setBrightnes(uint16_t value)
{
    this->brightnes = value;
...
void DMD::scanDisplayBySPI()
{
    if (this->ready == 0)
    {
        ...
        // GPOC = (1 << pin_oe); //digitalWrite(pin_oe, LOW);
        int rowsize = DisplaysTotal << 2;
        int offset = rowsize * bDMDByte;
        for (int i = 0; i < rowsize; i++)
        {
            this->shiftOut(pin_dataR, pin_sck, bDMDScreenRAM[offset + i + row3]);
            this->shiftOut(pin_dataR, pin_sck, bDMDScreenRAM[offset + i + row2]);
            this->shiftOut(pin_dataR, pin_sck, bDMDScreenRAM[offset + i + row1]);
            this->shiftOut(pin_dataR, pin_sck, bDMDScreenRAM[offset + i]);
        }
        byte out = gpio | bDMDByte;
        ...
        shiftOutBeep(pin_dataR, pin_sck, (out << 1));
        GPOS = (1 << pin_strb); //digitalWrite(pin_strb, HIGH);
        GPOC = (1 << pin_strb); //digitalWrite(pin_strb, LOW);
        // GPOS = (1 << pin_oe);   //digitalWrite(pin_oe, HIGH);
        analogWrite(pin_oe, this->brightnes);

        if (buzer == HIGH)
        {
            if (millis() - previousMillis >= interval)
            {
                this->write(BUZER, LOW);
                buzer = LOW;
            }
        }
        this->ready--;
...
{
    buzer = HIGH;
    this->write(BUZER, HIGH);
    previousMillis = millis();
}
void DMD::write(byte pin, bool value)
{
