...
public:
  ...
  DMD(byte panelsWide, byte panelsHigh, byte sck, byte strb, byte dataR, byte enable);
  //virtual ~DMD();
  ...
  void setBrightnes(uint16_t value);

private:
  uint16_t brightnes; // enable;
  uint8_t pin_oe;     // enable;
  uint8_t pin_sck;    // sck;
  uint8_t pin_dataR;  // dataR;
  uint8_t pin_strb;   //strb;
  uint8_t gpio;       //strb;
  uint8_t ready;
  bool buzer;
