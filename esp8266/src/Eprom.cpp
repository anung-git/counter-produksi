...
void Eprom::writeInt(int deviceaddress, unsigned int eeaddress, uint16_t data)
{
    Wire.beginTransmission(deviceaddress);
    Wire.write((int)(eeaddress >> 8));   // MSB
    Wire.write((int)(eeaddress & 0xFF)); // LSB
    Wire.write((int)(data >> 8));        // MSB
    Wire.write((int)(data & 0xFF));      // LSB
    Wire.endTransmission();
    delay(10);
}
...
uint16_t Eprom::readInt(int deviceaddress, unsigned int eeaddress)
{
    uint16_t rdata = 0;
    Wire.beginTransmission(deviceaddress);
    Wire.write((int)(eeaddress >> 8));   // MSB
